...
import {
  ...
  BuildingOutlineState, IBuildingFactory, BuildingIcon, IBuilding, BuildingUiTexture,
...
export class Building extends Phaser.GameObjects.Image implements IBuilding, ITile {
  ...
  readonly shape: Phaser.Geom.Polygon = null;
  ...
  private updateActionArea() {
    ...
    const { perspective, height } = TILE_META;
    const d = this.getActionsRadius() * 2;
    const out = height * 2;

    this.actionsArea.setSize(d, d * perspective);
    this.actionsArea.setDepth(Level.GetDepth(this.y + TILE_META.height * 0.5, 1, d * perspective + out));
...
registerSpriteAssets(BuildingUiTexture, {
  width: TILE_META.width,
  height: TILE_META.height,
});
